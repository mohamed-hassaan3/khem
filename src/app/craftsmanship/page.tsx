import type { Metadata } from "next";
import Image from "next/image";
import Link from "next/link";

import Reveal from "@/src/components/animation/Reveal";
import {
  getCraftStats,
  getCraftSteps,
  getMasterPerfumerQuote,
} from "@/src/services/content";

/** ISR, 1 hour — editorial copy, aligned with the other content routes. */
export const revalidate = 3600;

export const metadata: Metadata = {
  title: "Craftsmanship",
  description:
    "Six stages, four continents, one uncompromising standard — sourcing, formulation, mouth-blown flacons, hand filling, packaging, and a final assessment by a human nose.",
  alternates: { canonical: "/craftsmanship" },
  openGraph: {
    title: "Craftsmanship | Mastery in Every Drop",
    description:
      "How a KHEM fragrance is made: sixteen months, hundreds of formulation trials, and a bottle assembled entirely by hand.",
  },
};

/** Stagger step between grid children, in seconds. */
const STAGGER_STEP = 0.1;

export default async function Craftsmanship() {
  const [steps, stats, quote] = await Promise.all([
    getCraftSteps(),
    getCraftStats(),
    getMasterPerfumerQuote(),
  ]);

  return (
    <div className="min-h-screen bg-background text-ivory">
      {/* ── HERO ───────────────────────────────────── */}
      <section className="relative flex h-screen min-h-150 items-center overflow-hidden bg-black">
        <Image
          src="https://images.unsplash.com/photo-1709666414115-47ecd5143293?w=1800&h=1100&fit=crop&auto=format"
          alt=""
          fill
          priority
          quality={85}
          sizes="100vw"
          className="object-cover brightness-25 saturate-50"
        />
        <div className="absolute inset-0 bg-linear-to-b from-background/40 to-background/70" />

        <div className="relative z-10 max-w-2xl px-6 md:px-20 lg:px-30">
          <p className="eyebrow mb-5">The Art of KHEM</p>
          <h1 className="mb-8 font-heading text-5xl font-normal leading-none text-ivory sm:text-6xl md:text-7xl lg:text-8xl">
            Mastery in
            <br />
            <span className="text-gold">Every Drop</span>
          </h1>
          <div className="gold-line mb-8" />
          <p className="max-w-lg text-sm leading-loose text-ivory/50">
            Six stages. Four continents. One uncompromising standard. The
            creation of a KHEM fragrance is a devotional act — a process that
            takes months and demands perfection at every step.
          </p>
        </div>

        <div className="absolute bottom-12 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 md:flex">
          <p className="text-[9px] uppercase tracking-[0.3em] text-ivory/25">
            Explore the Process
          </p>
          <div className="h-15 w-px bg-linear-to-b from-gold/50 to-transparent" />
        </div>
      </section>

      {/* ── STAT BAND ──────────────────────────────── */}
      <section className="border-b border-border bg-surface">
        <div className="mx-auto grid max-w-7xl grid-cols-2 lg:grid-cols-4">
          {stats.map((stat, index) => (
            <Reveal
              key={stat.id}
              delay={index * STAGGER_STEP}
              // Dividers by index: never on the last column of the current
              // grid, so the 2-col mobile layout leaves no dangling border.
              className={`border-border px-6 py-12 text-center md:px-12 md:py-14 ${
                index % 2 === 0 ? "border-r" : index === 1 ? "lg:border-r" : ""
              } ${index < 2 ? "border-b lg:border-b-0" : ""}`}
            >
              <p className="mb-2.5 font-heading text-4xl font-semibold text-gold sm:text-5xl md:text-6xl">
                {stat.value}
              </p>
              <p className="text-[11px] leading-relaxed tracking-[0.12em] text-ivory/35">
                {stat.label}
              </p>
            </Reveal>
          ))}
        </div>
      </section>

      {/* ── PROCESS STEPS ──────────────────────────── */}
      {steps.map((step, index) => {
        const isImageLeft = index % 2 === 0;

        return (
          <section
            key={step.id}
            className={`border-t border-border ${
              isImageLeft ? "bg-background" : "bg-surface"
            }`}
          >
            <div className="grid grid-cols-1 lg:min-h-155 lg:grid-cols-2">
              {/* Image — always first on mobile, alternating from lg. */}
              <div
                className={`img-zoom relative aspect-4/3 overflow-hidden lg:aspect-auto ${
                  isImageLeft ? "" : "lg:order-2"
                }`}
              >
                <Image
                  src={step.image.url}
                  alt={step.image.alt}
                  fill
                  sizes="(min-width: 1024px) 50vw, 100vw"
                  className="object-cover brightness-55 saturate-65"
                />
                <span
                  aria-hidden="true"
                  className={`absolute top-8 font-heading text-7xl font-bold leading-none text-gold/15 md:top-10 md:text-8xl ${
                    isImageLeft ? "left-8 md:left-10" : "right-8 md:right-10"
                  }`}
                >
                  {step.number}
                </span>
              </div>

              {/* Text */}
              <div className="flex flex-col justify-center px-6 py-16 md:px-16 lg:py-20">
                <Reveal>
                  <p className="eyebrow mb-3">Step {step.number}</p>
                  <h2 className="mb-2.5 font-heading text-2xl font-normal leading-snug text-ivory sm:text-3xl md:text-4xl">
                    {step.title}
                  </h2>
                  <p className="mb-8 font-heading text-sm italic tracking-wide text-gold/70">
                    {step.subtitle}
                  </p>
                  <div className="gold-line mb-8" />
                  <p className="text-sm leading-loose text-ivory/50">
                    {step.body}
                  </p>
                </Reveal>
              </div>
            </div>
          </section>
        );
      })}

      {/* ── MASTER PERFUMER QUOTE ──────────────────── */}
      {quote ? (
        <section className="relative min-h-100 border-t border-border md:h-[60vh]">
          <Image
            src="https://images.unsplash.com/photo-1738664926458-d8ca7f56549f?w=1800&h=700&fit=crop&auto=format"
            alt=""
            fill
            sizes="100vw"
            className="object-cover brightness-30"
          />
          <div className="absolute inset-0 flex items-center justify-center">
            <Reveal className="max-w-3xl px-6 py-20 text-center md:px-20">
              <div className="gold-line mx-auto mb-9" />
              <p className="mb-7 font-heading text-xl italic leading-relaxed text-ivory sm:text-2xl md:text-3xl">
                &ldquo;{quote.quote}&rdquo;
              </p>
              <p className="mb-1.5 font-heading text-xs tracking-[0.2em] text-gold">
                {quote.author}
              </p>
              <p className="text-[11px] tracking-[0.1em] text-ivory/35">
                {quote.authorTitle}
              </p>
              <div className="gold-line mx-auto mt-9" />
            </Reveal>
          </div>
        </section>
      ) : null}

      {/* ── CTA ────────────────────────────────────── */}
      <section className="border-t border-border bg-background px-6 py-24 text-center md:px-20 md:py-30">
        <Reveal className="mx-auto max-w-xl">
          <p className="eyebrow mb-5">The Result</p>
          <h2 className="mb-5 font-heading text-2xl font-normal text-ivory sm:text-3xl md:text-4xl">
            Fragrances Worthy of History
          </h2>
          <p className="mb-11 text-[13px] leading-loose text-ivory/40">
            Sixteen months of work. Hundreds of decisions. One bottle. Discover
            the fragrances that justify every step of this process.
          </p>
          <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
            <Link href="/collections" className="btn-luxury btn-luxury-fill">
              Shop Collections
            </Link>
            <Link href="/ingredients" className="btn-luxury">
              Our Ingredients
            </Link>
          </div>
        </Reveal>
      </section>
    </div>
  );
}
